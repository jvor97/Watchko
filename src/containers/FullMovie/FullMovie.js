...
class FullMovie extends Component {
    ...
    loadMovieDataHandler(){
        if (this.props.match.params.id) {
            if (this.props.match.params.id != this.state.previousID) {
                axios.get('https://api.themoviedb.org/3/movie/' + this.props.match.params.id + '?api_key=65777f92529c3462f958232f137b357f&language=en-US&page=1&fbclid=IwAR3WdGpp9ZHMyGn4Vyni4MFF0hpc-Kfvyyj9PLnyueheoQ0o3YIPcmSL5Dk&language=en-US').then(response =>
                this.setState({
    ...
    render(){
        ...
        if (this.state.selectedMovie) {
            let genres = this.state.selectedMovie.genres.map(genre => genre.name + ' | ');
            console.log(genres);
